...
public class OceanOfCubesEffect : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            //buffer with one empty element
            collidersBuffer = new ComputeBuffer(1, sizeof(float) * (3 + 1), ComputeBufferType.Default); //position + influence
        }
    }

    void Update()
    {
        if (appendVertexBuffer == null) return;
