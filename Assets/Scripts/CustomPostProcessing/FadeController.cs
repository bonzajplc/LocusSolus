...
public class FadeController : MonoBehaviour
{
    ...
    public IEnumerator FadeWithAction(float duration, Color fadeOutColor, Color fadeInColor, LookAtObject.ProcessObjectMethod ProcessObject, GameObject ProcesObjectArg)
    {
        LookAtObject.lockCameraRaycast = true;
        yield return StartCoroutine(BeginFade(fadeOutColor, fadeInColor, duration));

        ProcessObject(ProcesObjectArg);

        yield return StartCoroutine(BeginFade(fadeInColor, fadeOutColor, duration));
        LookAtObject.lockCameraRaycast = false;
    }
